...
//                todo output your page here
                ...
                switch (function) {
                    case "new":{
                        ...
                        if(check_player == 0){
                            ...
                            getServletContext().getRequestDispatcher("/coaches/players_new.jsp?email="+email).forward(request, response);
                        }else{
                            Connection conn_add = null;
                            if(conn_add == null){
                                Connection conn_id = Konnect.getNAFAConnection();
                                conn_add = Konnect.getNAFAConnection();
                                String playerid = AutoIdDAO.getNextUniqueAutoID("cPlayerID", "PLY", "8", AppData.PLAYERS, conn_id);
                                DataAccessObject dac = new DataAccessObject();
                                dac.setColumnValues(playerid, teamid, fullname, emails, phone, gender, dob, address, stateid, grade, position, height, weight, special, remark, date,"player_profile.png");
                                ...
                                if(add == 0){
                                    ...
                                    getServletContext().getRequestDispatcher("/coaches/players_teams.jsp?email="+email).forward(request, response);
                                }else{
                                    request.setAttribute("portal", "1");
                    ...
                    case "edit":{
                        ...
                        if(conn_edit == null){
                            conn_edit = Konnect.getNAFAConnection();
                             DataAccessObject dac = new DataAccessObject();
                             dac.setColumnNames("iTeamID","vFullName","vEmail","cPhone","cGender","dDOB","vAddress","cState","vClass","vPosition","iHeight","iWeight","cSpecial","vAdditionalInfo");
                             dac.setColumnValues(teamid, fullname, emails, phone, gender, dob, address, stateid, grade, position, height, weight, special, remark);
                             ...
                             if(edit == 0){
                                ...
                                getServletContext().getRequestDispatcher("/coaches/players_teams.jsp?i="+email).forward(request, response);
                             }else{
                                request.setAttribute("portal", "1");
                                request.setAttribute("msg", "Player's Changes coauld not be saved successfully. kindly ensure the correct info is submitted.");
                                getServletContext().getRequestDispatcher("/coaches/players_new.jsp?i="+email).forward(request, response);
                             }
                        }
                        conn_edit.close();
                        break;
            ...
            Logger.getLogger(CoachPlayerServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    } 
