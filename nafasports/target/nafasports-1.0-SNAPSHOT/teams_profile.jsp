<%-- 
    Document   : teams_profile
    Created on : 7 Mar 2023, 3:57:22 PM
    Author     : josiah
--%>
...
<html>
    <head>
        <%
            ...
            if(id == null){
                ...
                teamname = TeamDAO.getTeamName(id);
                if(teamname.equals("")){
                    response.sendRedirect(request.getContextPath());
                }
            }
        ...
        <title>Welcome to NAFA | <%= teamname %>'s profile</title>
        <meta charset="UTF-8">
    ...
    <body>
        ...
        <section class="bg-light p-5 height-120px" id="navigation-banner">
            <div class="centralize w-50 text-center text-bold"><a href="./" class="text-decoration-none">Home</a> / <a href="teams.jsp" class="text-decoration-none">Teams</a> / <%= teamname %>'s Profile</div>
        </section>
        <section class="p-5">
            <div class="container">
                <div class="row mb-5">
                    <div class="col-12">
                        <h3><%= teamname %>'s Profile</h3>
                    </div>
                </div>
                <div class="row">
                    <div class="col-lg-8 col-md-8 col-sm-12">
                        <%
                            ...
                            if(id == null){}else{
                                String query = "select * from "+AppData.TEAMS+" where iTeamID = '"+id+"'";
                                DataAccessObject dao = ReportDynaDAO.loadRecord(query);
                                if(dao.next()){
                                    String teamid = dao.getString(1);
                                    String team = dao.getString(2);
                                    String code = dao.getString(3);
                                    String stateid = dao.getString(4);
                                    String cate = dao.getString(5);
                                    String logo = dao.getString(6);
                                    String districtid = dao.getString(7);
                        ...
                        <div class="row mb-5">
                            <div class="col-lg-3 col-md-3 col-sm-12">
                                <div class="box rounded height-200px w-200px block centralize">
                            ...
                            <div class="col-lg-9 col-md-9 col-sm-12">
                                <div class="block p-2">Name: <%= team.toUpperCase() %> (<%= code.toUpperCase() %>) </div>
                                <div class="block p-2">Location: <%= district %> <%= state.toUpperCase() %> STATE</div>
                                <div class="block p-2">Category: <%= cate.toUpperCase() %></div>
                                <div class="block p-2">Coach: <%= coach.toUpperCase() %></div>
                            </div>
                        </div>
                        <%
                            int total_team = 0;
                            String query_players = "select * from "+AppData.PLAYERS+" where iTeamID = '"+teamid+"'";
                            total_team = PlayerDAO.getTotalTeamPlayers(teamid);

                            DataAccessObject dac = null;
                            dac = ReportDynaDAO.loadRecord(query_players);
                            if(dac.next()){
                                dac.previous();
                                int j = 1;
                                int i = 0;
                                while(dac.next()){
                                    String playid = dac.getString(1);
                                    String teamsid = dac.getString(2);
                                    String fullname = dac.getString(3);
                                    String email = dac.getString(4);
                                    String phone = dac.getString(5);
                                    String gender = dac.getString(6);
                                    String dob = dac.getString(7);
                                    String position = dac.getString(11);
                                    String image = dac.getString(17);

                                    String paths = domain+"images/"+image;
                        ...
                        <%            
                            ...
                            <div class="col-lg-2 col-md-2 col-sm-12 p-2 ">
                                <a href="players_profile.jsp?id=<%= playid %>" class="text-decoration-none text-black">
                                    <div class="box rounded height-100px w-100px block centralize">
                                        <div class="block w-100 height-100" id="">
                                            <img src="<%= paths %>" class="w-100 h-100 border-radius-50" alt="alt"/>
                                        </div>
                                    </div>
